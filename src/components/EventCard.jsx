...
const EventCard = ({ name, image, link }) => {
  return (
    <Link href={link ? link : ""}>
      <div className="event rounded-lg my-5 lg:mx-20 border-2 pt-2 flex flex-col justify-center w-40 lg:w-52 h-auto border-gray-400">
        <ExportedImage alt={name} className="w-auto h-52 " src={image} />
        <h2 className="text-2xl break-before-avoid mx-4">{name}</h2>
